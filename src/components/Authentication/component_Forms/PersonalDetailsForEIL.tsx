...
export default function PersonalDetailsForEIL({ handleNext , setObjForSignInComonent}: propsType) {
  ...
  const handleChageInValue = (event: any) => {
    ...
    objForEstablishedIndustryLeaders[name] = value;
  }
  ...
  const handleChangeInOption=(event : any)=>{
    ...
    if(name === 'industry'){
      ...
      if(value !== ''){
        ...
        objForEstablishedIndustryLeaders["specialization"] = "";
        setIndustry(null);
      }
    }
  }
  ...
  const handleClick = async () => {
    try {
      await PersonalDataVarificationForEIL(dataForPersonalDetails);
      let Schema =  SchemaForEILObj(dataForPersonalDetails)
  ...
  useEffect(()=>{
    setDataForPersonalDetails(objForEstablishedIndustryLeaders)
  }, [])
